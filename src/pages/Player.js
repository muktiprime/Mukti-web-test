...
const Player = (props) => {
  ...
  const handleProgress = (changeState) => {
    if (
      videoInfo.skip_start &&
      videoInfo.skip_end &&
      changeState.playedSeconds >= convertSeconds(videoInfo.skip_start) &&
      changeState.playedSeconds <= convertSeconds(videoInfo.skip_end)
    ) {
      setShowSkip(true);
    } else {
      setShowSkip(false);
    ...
    if (
      videoInfo.next_episode &&
      changeState.playedSeconds >= convertSeconds(videoInfo.next_episode)
    ) {
      setShowNext(true);
    } else {
      setShowNext(false);
